...
function getPolynom(...args) {
  ...
  return (x) => {
    ...
    for (let i = 0; i < args.length; i += 1) {
      answer += args[i] * x ** (args.length - i - 1);
    }
    return answer;
  };
}
...
function retry(func, attempts) {
  return () => {
    for (let counter = 0; counter < attempts; counter += 1) {
      try {
        return func();
      } catch (e) { // Empty block statement.eslint no-empty
      }
    }
    return undefined;
  };
...
function logger(func, logFunc) {
  return (...args) => {
    const stringArgs = args.map((el) => JSON.stringify(el));
    // actual expected,test,1,0 - expected ["expected","test",1] !!!
